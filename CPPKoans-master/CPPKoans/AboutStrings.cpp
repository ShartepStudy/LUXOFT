...
void aboutCPPStrings(){
	...
	expectThat("C++ strings are mutable.", "big Bang theory",s);
}
void aboutStringAssignmentCreatesACopy(){
	string s1 = "hello";
	string s2 = s1;
	s2[1] = 'a';
	expectThat("s1 is not changed","hello",s1);
	expectThat("but s2 of course is changed","hallo",s2);
}
...
void aboutStringsAsParametersBeingCopied(){
	...
	expectThat("s1 is not changed by modification in function","hello",s1);
	expectThat("but the return value s2 contains the change","hallo",s2);
}
...
void aboutPassingAsReferenceMutatesTheOriginalString(){
	string s1 = "hello";
	mutateString(&s1);
	expectThat("if passed by reference, the string is modified","Hullu",s1);
}

void aboutTheConcatenationPitfall(){
	string s1 = "a ";
	expectThat("+ if overloaded to do string concatenation on C++ strings.","a cat",s1+"cat");
	expectThat("can also be used with chars.","a c",s1+'c');
    ...
    expectThat("in fact, it's a pointer manipulation II","6789c",string(s2+16));
